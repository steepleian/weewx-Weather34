...
for ($i = 0;$i < $cnt;$i++)
{
    $eqtitle[$i] = explode(" ", str_replace("  ", " ", $parsed_json['rss']['channel']['item'][$i]['title']) , 3) [2];
    $magnitude[$i] = number_format(explode(" ", str_replace("  ", " ", $parsed_json['rss']['channel']['item'][$i]['emsc:magnitude'])) [1], 1);
    $depth[$i] = $parsed_json['rss']['channel']['item'][$i]['emsc:depth'];
    ...
    $eventime[$i] = date("D j M Y H:i:s", (strtotime($parsed_json['rss']['channel']['item'][$i]['emsc:time'])));
...
else if($theme==="light"){$text1="black";$background1="whitesmoke";}
$forecastime = filemtime ('jsondata/eq.txt');?>
<div class="weather34darkbrowser" style="color:<?php echo $text1 ?>;"url="Recent Earthquakes"></div>  
<main class="grid">
<?php
for ($j = 0;$j < $cnt;$j++)
{
