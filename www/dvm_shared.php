...
// Speed
function ktsTokmh(&$dvm, $field){
	if(!isset($dvm[$field])) return;
	$dvm[$field] = number_format(1.852*$dvm[$field],1);
}

function ktsTomph(&$dvm, $field){
	if(!isset($dvm[$field])) return;
	$dvm[$field] = number_format(1.150779*$dvm[$field],1);
}

function ktsToms(&$dvm, $field){
	if(!isset($dvm[$field])) return;
	$dvm[$field] = number_format(0.514444*$dvm[$field],2);
}

function mphTokmh(&$dvm, $field){
	if(!isset($dvm[$field])) return;
	$dvm[$field] = number_format(1.609344*$dvm[$field],1);
}

function mphTokts(&$dvm, $field){
	if(!isset($dvm[$field])) return;
	$dvm[$field] = number_format(0.868976*$dvm[$field],1);
}

function mphToms(&$dvm, $field){
	if(!isset($dvm[$field])) return;
	$dvm[$field] = number_format(0.44704*$dvm[$field],1);
}

function kmhTokts(&$dvm, $field){
	if(!isset($dvm[$field])) return;
	$dvm[$field] = number_format(0.5399568*$dvm[$field],1);
}

function kmhTomph(&$dvm, $field){
	if(!isset($dvm[$field])) return;
	$dvm[$field] = number_format(0.621371*$dvm[$field],1);
}

function kmhToms(&$dvm, $field){
	if(!isset($dvm[$field])) return;
	$dvm[$field] = number_format(0.2777778*$dvm[$field],1);
}

function msTokmh(&$dvm, $field){
	if(!isset($dvm[$field])) return;
	$dvm[$field] = number_format(3.6*$dvm[$field],1);
}

function msTokts(&$dvm, $field){
	if(!isset($dvm[$field])) return;
	$dvm[$field] = round((float)(1.943844*$dvm[$field]), 1);
}

function msTomph(&$dvm, $field){
	if(!isset($dvm[$field])) return;
	$dvm[$field] = number_format(2.236936*$dvm[$field], 1);
}

function anyWtomph($field){
	if($dvm["wind_units"]=='mph') return $field;
	if($dvm["wind_units"]=='kts') return number_format(1.150779*$field,1);
	if($dvm["wind_units"]=='kmh') return number_format(0.621371*$field,1);
	if($dvm["wind_units"]=='ms')  return number_format(2.236936*$field,1);
}

function anyWtoms($field){
	if($dvm["wind_units"]=='ms')  return $field;
	if($dvm["wind_units"]=='kts') return number_format(0.514444*$field,1);
	if($dvm["wind_units"]=='kmh') return number_format(0.2777778*$field,1);
	if($dvm["wind_units"]=='mph') return number_format(0.44704*$field,1);
}

// Temperature
function cToF(&$dvm, $field){
	if(!isset($dvm[$field])) return;
	$dvm[$field] = cToFDirect($dvm[$field]);
}

function fToC(&$dvm, $field){
	if(!isset($dvm[$field])) return;
	$dvm[$field] = fToCDirect($dvm[$field]);
}

function cToFrel(&$dvm, $field){
	if(!isset($dvm[$field])) return;
	$dvm[$field] = round((float)((9/5)*($dvm[$field])), 1);
}

function fToCrel(&$dvm, $field){
	if(!isset($dvm[$field])) return;
	$dvm[$field] = round((float)((5/9)*($dvm[$field])), 1);
}
...
// Depth
function mmToin(&$dvm, $field){
	if(!isset($dvm[$field])) return;
	$dvm[$field] = number_format(0.03937008*$dvm[$field], 2);
}

function inTomm(&$dvm, $field){
	if(!isset($dvm[$field])) return;
	$dvm[$field] =number_format(25.4*$dvm[$field], 1);
}
...
$dvm34menuicon='<svg width="32pt" height="32pt" viewBox="0 0 1003 799" id="weather34 menu">
...
$copyYear = 2015;$curYear = date('Y');$dvm34copyrightcredit='&copy; weather34.com original CSS/SVG/PHP '.$copyYear . (($copyYear != $curYear) ? '-' . $curYear : 'Copyright');
